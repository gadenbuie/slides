...
fake_chunk(
  ...
  paste0(
    show_from(0, '<span class="text-yellow">{.titlecase age}</span> <span class="text-mint">{type}</span>&nbsp;'),
    show_from(1, 'in <span class="text-red">{county_size}</span> counties\n'),
...
::: {.epoxy-ex-output .px-7}
[[Infant]{.bg-yellow} [day care]{.bg-mint}]{.fragment}
[in [very large]{.bg-red} counties]{.fragment}
...
fake_chunk(
  ...
  paste0(
    ...
    'ranks {.ordinal cost_rank} in childcare costs\n',
    'at {.dollar max_med_cost} per year,',
    show_from(0, '<span class="text-gray-12">about <span class="text-blue">{.dollar {.cpm max_med_cost}}</span> per month</span>'),
    '\nor {.percent mhi_pct} of median household income.'
  )
)
```
...
[about [$1,284.75]{.bg-blue} per month]{.fragment .text-gray-12 show-from=0 style="--display: inline"}
...
::: {.fragment .hide-code-line-numbers show-on="1"}
```{r}
#| echo: true
#| eval: false
...
::: {.epoxy-ex-output .px-7 .text-gray-6}
```{r}
enclassen <- function(class) {
  function(transformer = glue::identity_transformer) {
    function(text, envir) {
...
epoxy_use_chunk(
  ...
  .transformer = list(epoxy_transform_inline, enclassen("text-gray-12"))
)
```
:::
...
epoxy_use_chunk(
  ...
  .transformer = list(epoxy_transform_inline, enclassen("text-orange"))
)
```
:::
